...
import { serviceCategoryService } from '../src/modules/services/services/service-category.service';
import { serviceService } from '../src/modules/services/services/service.service';
import { serviceAvailabilityService } from '../src/modules/services/services/service-availability.service';
import { DepositPolicyType, EmployeeAssignmentMode } from '@prisma/client';
...
async function runTests() {
  ...
  const memberA = await prisma.businessMember.create({
    data: {
      businessId: businessA.id,
      userId: user.id,
      status: 'ACTIVE',
  ...
  const memberB = await prisma.businessMember.create({
    data: {
      businessId: businessB.id,
      userId: user.id,
      status: 'ACTIVE',
  ...
  const branchA3 = await prisma.branch.create({
    data: {
      businessId: businessA.id,
  ...
  const catHair = await serviceCategoryService.createCategory(businessA.id, user.id, {
    name: 'Hair',
  ...
  try {
    await serviceCategoryService.createCategory(businessA.id, user.id, {
      name: 'Hair',
  ...
  const catHairB = await serviceCategoryService.createCategory(businessB.id, user.id, {
    name: 'Hair',
  ...
  const catNails = await serviceCategoryService.createCategory(businessA.id, user.id, {
    name: 'Nails',
  ...
  try {
    await serviceService.createService(businessA.id, user.id, {
      categoryId: catHair!.id,
      name: 'Haircut',
      durationMinutes: 0,
      price: 100,
      employeeAssignmentMode: EmployeeAssignmentMode.ANY_AVAILABLE,
      branchIds: [branchA1.id],
    });
  ...
  try {
    await serviceService.createService(businessA.id, user.id, {
      ...
      price: 100,
      employeeAssignmentMode: EmployeeAssignmentMode.ANY_AVAILABLE,
      depositPolicyType: DepositPolicyType.FIXED,
      depositAmount: 150,
      branchIds: [branchA1.id],
    });
  ...
  try {
    await serviceService.createService(businessA.id, user.id, {
      ...
      durationMinutes: 45,
      price: 200,
      employeeAssignmentMode: EmployeeAssignmentMode.CUSTOMER_CHOOSES,
  ...
  const serviceHaircut = await serviceService.createService(businessA.id, user.id, {
    categoryId: catHair!.id,
  ...
  const serviceManicure = await serviceService.createService(businessA.id, user.id, {
  ...
  const availA1 = await serviceAvailabilityService.isServiceAvailableAtBranch(serviceHaircut!.id, branchA1.id);
  const availA2 = await serviceAvailabilityService.isServiceAvailableAtBranch(serviceHaircut!.id, branchA2.id);
  ...
  const maniA1 = await serviceAvailabilityService.isServiceAvailableAtBranch(serviceManicure!.id, branchA1.id);
  const maniA2 = await serviceAvailabilityService.isServiceAvailableAtBranch(serviceManicure!.id, branchA2.id);
  ...
  await serviceCategoryService.updateCategoryBranchAssignment(catHair!.id, branchA1.id, user.id, false);
  const availA1AfterDeactivate = await serviceAvailabilityService.isServiceAvailableAtBranch(serviceHaircut!.id, branchA1.id);
  ...
  await serviceCategoryService.updateCategoryBranchAssignment(catHair!.id, branchA1.id, user.id, true);
  const availA1AfterReactivate = await serviceAvailabilityService.isServiceAvailableAtBranch(serviceHaircut!.id, branchA1.id);
  ...
  await serviceService.updateServiceBranchAssignment(serviceHaircut!.id, branchA1.id, user.id, false);
  const availA1ServiceDeactivated = await serviceAvailabilityService.isServiceAvailableAtBranch(serviceHaircut!.id, branchA1.id);
  ...
  await serviceService.updateServiceBranchAssignment(serviceHaircut!.id, branchA1.id, user.id, true);
